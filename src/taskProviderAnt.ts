...
const detectAntScripts = async (): Promise<Task[]> =>
{
    util.log("");
    util.log("detectAntScripts");
...
export const provideAntScripts = async (): Promise<Task[]> =>
{
    util.log("");
    util.log("provideAntScripts");
...
const readAntfile = async (uri: Uri): Promise<Task[]> =>
{
    const result: Task[] = [];
    const folder = workspace.getWorkspaceFolder(uri);
...
const findAllAntScripts = async (path: string): Promise<StringMap> =>
{
    let text: any = "";
    const scripts: StringMap = {};
    const buffer = await util.readFile(path);
...
const createAntTask = (target: string, cmdName: string, folder: WorkspaceFolder, uri: Uri): Task =>
{
    const getRelativePath = (folder: WorkspaceFolder, uri: Uri): string =>
    {
        if (folder) {
